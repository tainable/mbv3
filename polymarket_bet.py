...
def wrap_usdc_to_pusd(private_key: str, amount_usdc: float, polygon_rpc_url: str | None = None) -> None:
    """Wrap USDC.e into pUSD via Polymarket's CollateralOnramp.

    Steps:
      1. Approve CollateralOnramp to spend USDC.e (if not already done)
      2. Call CollateralOnramp.wrap(amount) → receive equivalent pUSD
    """
    from eth_account import Account
    ...
    amount_raw = int(amount_usdc * 1_000_000)  # USDC.e has 6 decimals

    usdc_e_bal = _get_erc20_balance(_USDC_LEGACY, address, rpc_list)
    print(f"  USDC.e balance : {usdc_e_bal:.6f}")
    if usdc_e_bal < amount_usdc - 1e-6:
        raise RuntimeError(
            f"Insufficient USDC.e: have {usdc_e_bal:.6f}, need {amount_usdc:.6f}"
        )
    ...
    tx_approve = {
        ...
        "to": _USDC_LEGACY, "value": 0, "data": approve_calldata,
        "chainId": _POLYGON_CHAIN_ID,
    }
    signed  = Account.sign_transaction(tx_approve, private_key)
    tx_hash = _rpc("eth_sendRawTransaction", ["0x" + signed.raw_transaction.hex()], rpc_list)
    print(f"  Approving CollateralOnramp...  tx: {tx_hash}")
    _wait_for_receipt(tx_hash, rpc_list=rpc_list)

    # Step 2 — call wrap(asset, to, amount)
    asset_padded  = bytes.fromhex("000000000000000000000000" + _USDC_LEGACY.lower().replace("0x", ""))
    ...
    tx_wrap = {
        "nonce": nonce + 1, "gasPrice": gas_price, "gas": 150_000,
        "to": _ONRAMP_CONTRACT, "value": 0, "data": wrap_calldata,
        "chainId": _POLYGON_CHAIN_ID,
    }
    signed  = Account.sign_transaction(tx_wrap, private_key)
    tx_hash = _rpc("eth_sendRawTransaction", ["0x" + signed.raw_transaction.hex()], rpc_list)
    print(f"  Wrapping {amount_usdc:.6f} USDC.e -> pUSD...  tx: {tx_hash}")
    _wait_for_receipt(tx_hash, rpc_list=rpc_list)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        metavar="USDC_E_AMOUNT",
        help=(
            "Wrap USDC.e into pUSD via Polymarket's CollateralOnramp (Polymarket V2 collateral). "
            "Example: --wrap 10.0  converts 10 USDC.e → 10 pUSD. "
    ...
    if not args.check and not args.approve and not args.bets and not args.cancel and args.wrap <= 0 and args.amount <= 0:
    ...
    if args.wrap > 0:
        print(f"Wrapping {args.wrap} USDC.e -> pUSD via Polymarket CollateralOnramp...")
        print("(You need a small amount of MATIC in your wallet for gas)\n")
        rpc_url = settings.polymarket.polygon_rpc_url
        try:
            wrap_usdc_to_pusd(private_key, args.wrap, polygon_rpc_url=rpc_url)
        except Exception as exc:
